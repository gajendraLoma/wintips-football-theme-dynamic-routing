...
import {
  BettingThreeInOneSectionProps,
  Category,
  CategoryItem
} from '@/types/interface/getHomepageTypo';
export default function BettingThreeInOneSection({
  ...
  const sections = [
    ...
    },
    {
      ...
      items: hasValidData(data?.category_middle)
        ? data.category_middle.post
        : null
    },
    {
      title: data?.category_right?.name,
      items: hasValidData(data?.category_right)
        ? data.category_right.post
        : null
    }
  ...
  return (
    <div className="py-8">
      <div className="grid gap-2 sm:gap-10 md:grid-cols-3">
        {sections.map((section, index) => {
          const featuredImage = getFullImageUrl(
            section.items[0].featured_image
          );
          return (
                ...
                {section.items.slice(1).map((item, subIndex) => {
                  return (
                    <li key={subIndex} className="py-2">
                      <Link
                        href={`/${item.slug}`}
                        className="flex hover:text-[#60a5fa]"
                      >
                        <svg
